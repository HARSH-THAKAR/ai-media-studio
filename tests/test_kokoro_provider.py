...
def _settings(root: Path) -> KokoroSettings:
    return KokoroSettings("local-voice", 1.0, "a", 24_000, None)
...
def _storyboard() -> ScriptResult:
    scene = Scene(1, "A test narration.", "test image", 3.0, "fade")
    return ScriptResult("test", "Test", "Hook", "CTA", (scene,), "test", "test", 0.1)
